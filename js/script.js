...
function hibaSzoveg(err) {
  ...
  switch (c) {
    ...
    case 'auth/requires-recent-login':
      return 'A jelszó módosításához jelentkezz ki, majd be újra, és próbáld rögtön.';
    default:
...
async function authLogin() {
  ...
  if (!email || !pass) {
    if (errEl) {
      errEl.textContent = 'Add meg az e-mail címet és a jelszót!';
  ...
  try {
    ...
    if (errEl) {
      errEl.textContent = hibaSzoveg(err);
      errEl.style.display = 'block';
    }
  }
  btn.disabled = false;
...
async function authRegister() {
  ...
  if (pass.length < 6) {
    if (errEl) {
      errEl.textContent = 'A jelszó legalább 6 karakter legyen.';
      errEl.style.display = 'block';
    }
    return;
  }
...
function authResetPassword() {
  ...
  if (!email) {
    if (errEl) {
      errEl.textContent = 'Add meg az e-mail címed a jelszó helyi visszaállításához.';
      errEl.style.display = 'block';
    }
    return;
  }
...
function setAuthMode(mode) {
  ...
  if (_authMode === 'register') {
    ...
    if (toggle)
      toggle.innerHTML =
        'Van már fiókod? <a href="#" onclick="setAuthMode(\'login\');return false">Belépés</a>';
    ...
    if (toggle)
      toggle.innerHTML =
        'Még nincs fiókod? <a href="#" onclick="setAuthMode(\'register\');return false">Regisztrálj</a>';
...
const I18N_HU_EN = {
  ...
  '+ Bevétel rögzítése': '+ Add income',
  '+ Költség rögzítése': '+ Add expense',
  '+ Megkeresés': '+ Inquiry',
  '+ Megkeresés rögzítése': '+ Add inquiry',
  '+ Megrendelés rögzítése': '+ Add order',
  '+ Rögzítés': '+ Add',
  Megnyitás: 'Open',
  ...
  'Számla készítése': 'Create invoice',
  '+ Számla': '+ Invoice',
  'Kifizetettnek jelöl': 'Mark as paid',
  Kész: 'Done',
  Profil: 'Profile',
  ...
  'Megkeresés dátuma': 'Inquiry date',
  Megnevezés: 'Description',
  'Megrendelés dátuma': 'Order date',
  Mennyiség: 'Quantity',
  Név: 'Name',
  ...
  Telefon: 'Phone',
  'Téma / rövid leírás': 'Subject / short description',
  ...
  'Ügyfél / forrás': 'Client / source',
  'Ügyfél kerete': 'Client budget',
  'Ügyfél típusa': 'Client type',
  'Üzenet / részletek (nem kötelező)': 'Message / details (optional)',
  Azonosító: 'ID',
  Bevétel: 'Income',
  'Beárazás (Ft)': 'Pricing',
  Eredmény: 'Result',
  ...
  Projekt: 'Project',
  Típus: 'Type',
  'Típus / téma': 'Type / subject',
  'Utolsó kapcsolat': 'Last contact',
  Vevő: 'Buyer',
  Ár: 'Price',
  Összeg: 'Amount',
  Ügyfél: 'Client',
  Üzenet: 'Message',
  Egységár: 'Unit price',
  'Nettó egységár': 'Net unit price',
  'Nettó összeg': 'Net amount',
  '< 100 000 Ft': '< 100,000 HUF',
  '100–300 000 Ft': '100–300,000 HUF',
  '300–600 000 Ft': '300–600,000 HUF',
  '600 000 Ft <': '600,000 HUF <',
  'Alanyi adómentes (nem ÁFA-körös)': 'VAT-exempt (not VAT-registered)',
  'Angol (€)': 'English (EUR)',
  Bankköltség: 'Bank fees',
  'Bemutatkozó weboldal': 'Portfolio website',
  'Betéti társaság (Bt.)': 'Limited partnership (Bt.)',
  'Egyedi webalkalmazás': 'Custom web app',
  Egyéb: 'Other',
  'Egyéni cég (EC)': 'Sole company (EC)',
  'Egyéni vállalkozó': 'Sole proprietor',
  'Eszköz / hardver': 'Equipment / hardware',
  'Fejlesztés alatt': 'In development',
  'Karbantartás / frissítés': 'Maintenance / update',
  Kifizetve: 'Paid',
  ...
  'Magyar (Ft)': 'Hungarian (HUF)',
  Magánszemély: 'Individual',
  'Marketing / hirdetés': 'Marketing / ads',
  'Mindkettő — a weboldal nyelvét követi (HU=Ft, EN=€)':
    'Both — follows the website language (HU=HUF, EN=EUR)',
  'Nem megadott': 'Not specified',
  'Nincs megadva': 'Not specified',
  'Nyilvános részvénytársaság (Nyrt.)': 'Public company (Nyrt.)',
  'Oktatás / tanfolyam': 'Education / course',
  'Szoftver / licenc': 'Software / license',
  Szövetkezet: 'Cooperative',
  Tanácsadás: 'Consulting',
  'Teszt verzió': 'Test version',
  'Tárhely / domain': 'Hosting / domain',
  'Utazás / üzemanyag': 'Travel / fuel',
  'Weboldal projekt': 'Website project',
  Webáruház: 'Webshop',
  ...
  'Bevételi napló': 'Income log',
  'Beérkezett megkeresések': 'Incoming inquiries',
  'E havi kiadások megoszlása': "This month's expense breakdown",
  'Folyamatban lévő projektek': 'Projects in progress',
  Függőben: 'Pending',
  'Függőben lévő számlák': 'Pending invoices',
  'Havi bevétel / kiadás — utolsó 6 hónap': 'Monthly income / expense — last 6 months',
  'Havi pénzforgalom': 'Monthly cash flow',
  'Havi átlag': 'Monthly average',
  'Havi összes': 'Monthly total',
  'Idei költségek': 'Expenses this year',
  'Idei árbevétel': 'Revenue this year',
  'Idei összes': 'Total this year',
  'Idei összes projekt': 'Total projects this year',
  'Kategóriák megoszlása': 'Category breakdown',
  Kifizetett: 'Paid',
  'Következő határidő': 'Next deadline',
  'Közelgő határidők': 'Upcoming deadlines',
  'Legnagyobb kategória': 'Largest category',
  Lejárt: 'Overdue',
  'Megjelenő mezők': 'Visible fields',
  'Projektek státusza': 'Project status',
  'Szolgáltatások (ár is)': 'Services (with price)',
  'Számlázható projektek': 'Billable projects',
  Tételek: 'Items',
  'Élő előnézet': 'Live preview',
  'Összes számla': 'All invoices',
  Ügyféllista: 'Client list',
  '🔔 Beérkező megkeresések': '🔔 Incoming inquiries',
  'Árak megjelenítése a szolgáltatásoknál': 'Show prices on services',
  'Bevétel rögzítése': 'Add income',
  'Megrendelés rögzítése': 'Add order',
  'Vállalkozási költség rögzítése': 'Add business expense',
  ...
  'Kiállítás dátuma': 'Issue date',
  'Teljesítés dátuma': 'Fulfilment date',
  'Fizetési mód': 'Payment method',
  Átutalás: 'Bank transfer',
  Fizetendő: 'Total due',
  'Fizetendő (bruttó)': 'Total due (gross)',
  'Nettó összesen': 'Net total',
  'fejlesztés + teszt': 'in development + test',
  'az idén rögzített': 'recorded this year',
  'aktív, még nem lejárt': 'active, not yet overdue',
  'árazásra vár': 'awaiting pricing',
  'Árbevétel − költségek (idei)': 'Revenue − expenses (this year)',
  'Idei aktív hónapok átlaga': 'Average of active months this year',
  'Következő határidő': 'Next deadline',
  Nincs: 'None',
  'Nincs adat': 'No data',
  'Nincs közelgő határidő.': 'No upcoming deadlines.',
  'A weboldalakról érkezett rendelési űrlapok — itt árazza be és kezeli őket':
    'Order forms received from your websites — price and manage them here',
  'Fejlesztés alatt és teszt verzió': 'In development and test version',
  'Új megkeresés': 'New inquiry',
  'Ajánlat elküldve': 'Quote sent',
  'Ajánlat elfogadva': 'Quote accepted',
  'Projektté alakítva': 'Converted to project',
  Törölve: 'Deleted',
  Karbantartás: 'Maintenance',
  Megrendelés: 'Order',
  'Automatikusan könyvelt': 'Booked automatically',
  Számla: 'Invoice',
  'Bemutatkozó oldal': 'Landing page',
  Webshop: 'Web shop',
  ...
  'Még nincs rögzített megkeresés. Új felvétele a „+ Megkeresés" gombbal.':
    'No inquiries recorded yet. Add one with the “+ Inquiry” button.',
  ...
  'Válaszd ki a vállalkozási formát, majd a hozzá tartozó adózási módot — ezt később az adószámításnál használjuk.':
    'Choose your business form, then the matching taxation type — we use this for tax calculation.',
  ...
  'Ez határozza meg, hogyan jelenik meg az ÁFA a számlán: alanyi adómentesnél „AAM" (áfa nélkül), ÁFA-körösnél nettó + ÁFA + bruttó bontásban. Az űrlapon megadott árakat nettóként kezeli.':
    'This determines how VAT appears on the invoice: for VAT-exempt “AAM” (no VAT), for VAT-registered a net + VAT + gross breakdown. Prices entered on the form are treated as net.',
  ...
  'angol szöveg + €': 'English text + EUR',
  '. Ez rögzített — nincs teendőd vele.': '. This is fixed — nothing to do.',
  'Magánszemély / Vállalkozó választó + cégadatok (cégnév, adószám)':
    'Individual / Company selector + company details (name, VAT number)',
  'Telefon mező': 'Phone field',
  'Üzenet mező': 'Message field',
  'Kívánt határidő mező': 'Preferred deadline field',
  'Tervezett keret mező': 'Budget field',
  'E-mail értesítők': 'Email notifications',
  'Automatikus: minden leadott rendelésnél a rendszer e-mailt küld — neked egy értesítőt (a regisztrációs e-mail címedre), az ügyfélnek egy visszaigazolást (a weboldal nyelvén). Nincs vele teendőd.':
    'Automatic: for every submitted order the system sends emails — a notification to you (to your registration email) and a confirmation to the customer (in the website language). Nothing to do.',
  'Így fog kinézni az űrlap (a te weboldalad stílusát veszi majd fel).':
    'This is how the form will look (it will take on your website’s style).',
  'Ezt a kódot': 'This code',
  'elég egyetlen egyszer': 'just once is enough',
  'beilleszteni a weboldaladba. Utána, ha itt módosítasz valamit és mentesz, a weboldaladon lévő űrlap magától frissül — nem kell újramásolni. A beérkező rendelés a „Megrendelések" fülön jelenik meg. Bármilyen oldalon működik (sima HTML, WordPress „Egyéni HTML" blokk, stb.).':
    'to paste into your website. After that, if you change something here and save, the form on your website updates automatically — no need to re-copy. The incoming order appears on the “Orders” tab. Works on any page (plain HTML, WordPress “Custom HTML” block, etc.).',
  'Mentve ✓ — a weboldaladon lévő űrlap magától frissül.':
    'Saved ✓ — the form on your website updates automatically.',
  'Vállalkozási adatok mentve ✓': 'Business details saved ✓',
  'Profil mentve ✓': 'Profile saved ✓',
  'Cégadatok mentve ✓': 'Company details saved ✓',
  'Mentve ✓': 'Saved ✓',
  'Jelszó módosítva ✓': 'Password changed ✓',
  'Kód a vágólapra másolva ✓': 'Code copied to clipboard ✓',
  ...
  Összes: 'Total',
  Havi: 'Monthly',
  'Ez csak előnézet — a valódi űrlap a weboldaladon küld.':
    'This is only a preview — the real form on your site sends the order.'
};
...
function normalizeFormConfig() {
  ...
  ['phone', 'message', 'deadline', 'budget'].forEach((k) => {
    if (typeof fc.fields[k] !== 'boolean') fc.fields[k] = k === 'phone' || k === 'message';
  });
}
...
function migrateOrderNumbers() {
  ...
  state.orders.forEach((o) => {
    ...
    if (m) {
      const y = m[1],
        n = parseInt(m[2], 10);
      if (n > (state.orderNumByYear[y] || 0)) state.orderNumByYear[y] = n;
    }
  });
  state.orders
    .filter((o) => !o.num)
...
function orderPriceHuf(o) {
  return toHuf(o.price || 0, o.currency, o.fxRate);
}
    ...
    fetch('https://api.frankfurter.app/latest?from=EUR&to=HUF')
      ...
      .then((d) => {
        if (d && d.rates && d.rates.HUF) {
          state.fxEurHuf = d.rates.HUF;
          if (typeof publishFormConfig === 'function') publishFormConfig();
        }
      })
      .catch(() => {});
  } catch (e) {}
...
function showLoadError(err) {
  ...
  if (!box) {
    ...
    box.style.cssText =
      'position:fixed;inset:0;z-index:9999;display:flex;align-items:center;justify-content:center;padding:24px;background:var(--surface,#111);color:var(--text,#eee);font-family:inherit';
    box.innerHTML =
...
async function resetAllData() {
  if (
    !(await uiConfirm(
      'Biztosan törlöd az ÖSSZES adatodat ebben a böngészőben (bevételek, kiadások, projektek, megkeresések, számlák)? Ez a művelet nem visszavonható — előtte érdemes fájlba menteni!',
      {
    ...
    return;
  if (
    !(await uiConfirm('Utolsó megerősítés: minden rögzített adat véglegesen törlődik. Folytatod?', {
...
window.populateAccountForms = function () {
  ...
  if (keyEl) keyEl.value = (state && state.inboxKey) || '';
  const snipEl = document.getElementById('inbox-snippet');
  if (snipEl) snipEl.value = inboxEmbedSnippet();
...
function saveAccountProfile() {
  ...
  try {
    LocalStore.updateProfileName(name);
  } catch (e) {}
...
(function () {
  ...
  function erow(label, val) {
    ...
    return '<tr><td style="padding:10px 0;border-bottom:1px solid #e4eaf5;font-size:12px;color:#5d6b85;vertical-align:top;white-space:nowrap">' + esc(label) +
           '</td><td style="padding:10px 0 10px 18px;border-bottom:1px solid #e4eaf5;font-size:14px;color:#171c28;font-weight:600;vertical-align:top;text-align:right">' + esc(val) + '</td></tr>';
  }
...
function formPreviewHtml() {
  ...
  const svcPrice = (s) =>
    en
      ? Number(s.priceEur) > 0
        ? Number(s.priceEur)
        : rate > 0
          ? Math.round((Number(s.price) || 0) / rate)
          : 0
      : Number(s.price) || 0;
  ...
  const L = en
    ? {
        ...
        mo: ' / mo',
        moWord: ' (monthly)',
        note: 'This is only a preview — the real form on your site sends the order.'
      }
    : {
        name: 'Név *',
        email: 'E-mail *',
        ...
        mo: ' / hó',
        moWord: ' (havi)',
        note: 'Ez csak előnézet — a valódi űrlap a weboldaladon küld.'
      };
  ...
  if (fc.business) {
    P.push(
      '<select class="fp-inp" onchange="var b=document.getElementById(\'fp-biz\');if(b)b.style.display=this.value===\'' +
        esc(L.biz) +
        "'?'flex':'none'\"><option>" +
        esc(L.priv) +
        '</option><option>' +
    ...
    P.push(
      '<span id="fp-biz" style="display:none;flex-direction:column;gap:10px"><input class="fp-inp" placeholder="' +
        esc(L.co) +
        '"><input class="fp-inp" placeholder="' +
        esc(L.tax) +
        '"></span>'
    );
  }
  if (services.length) {
    ...
    P.push(
      '<select class="fp-inp"><option>' +
        esc(L.choose) +
        '</option>' +
        services.map((s) => '<option>' + label(s) + '</option>').join('') +
        '</select>'
    );
  } else {
  ...
  if (f.message)
    P.push('<textarea class="fp-inp" placeholder="' + esc(L.message) + '"></textarea>');
  P.push(
    '<button type="button" class="btn" style="width:100%" onclick="var n=document.getElementById(\'fp-note\');if(n){n.textContent=' +
  ...
  P.push(
    '<div id="fp-note" style="font-size:11.5px;color:var(--muted);text-align:center;min-height:14px"></div>'
  );
  return P.join('');
}
...
function fallbackCopy(text, done) {
  try {
    ...
    setAcctNote('acct-inbox-note', 'A másolás nem sikerült — jelöld ki kézzel.', true);
  }
}
...
function updateService(i, field, val) {
  ...
  if (field === 'price') fc.services[i].price = Number(String(val).replace(/[^\d]/g, '')) || 0;
    ...
    fc.services[i].priceEur = Number(String(val).replace(/[^\d]/g, '')) || 0;
  else if (field === 'period') fc.services[i].period = val === 'monthly' ? 'monthly' : 'once';
  else if (field === 'priceMode')
    fc.services[i].priceMode = ['exact', 'from', 'free'].indexOf(val) >= 0 ? val : 'exact';
...
function addService() {
  state.formConfig.services.push({
    name: '',
    nameEn: '',
    cat: '',
    catEn: '',
...
function readFormToggles() {
  ...
  const b = chk('fc-business');
  if (b !== undefined) fc.business = b;
  const sp = chk('fc-showprices');
  if (sp !== undefined) fc.showPrices = sp;
  fc.formLang = 'both';
  ['phone', 'message', 'deadline', 'budget'].forEach((k) => {
    const v = chk('fc-field-' + k);
    if (v !== undefined) fc.fields[k] = v;
  });
}
function updateFormOutputs() {
  ...
  const uidEl = document.getElementById('fc-uid');
  if (uidEl) uidEl.value = inboxTargetUid();
}
...
function saveFormConfig() {
  ...
  if (note) {
    note.textContent = 'Mentve \u2713 — a weboldaladon lévő űrlap magától frissül.';
    setTimeout(() => {
      note.textContent = '';
    }, 4500);
  }
...
function copyFormSnippet() {
  ...
  const done = () => {
    const n = document.getElementById('fc-save-note');
    if (n) {
      n.textContent = 'Kód a vágólapra másolva \u2713';
  ...
  try {
    if (navigator.clipboard && navigator.clipboard.writeText)
      navigator.clipboard
        .writeText(text)
        .then(done)
        .catch(() => fallbackFormCopy(text, done));
    else fallbackFormCopy(text, done);
  } catch (e) {
    fallbackFormCopy(text, done);
  }
}
...
function showTab(id) {
  ...
  if (id === 'account') {
    ...
    if (typeof window.populateAccountForms === 'function') {
      try {
        window.populateAccountForms();
      } catch (e) {}
    }
  }
  if (id === 'formcfg' && typeof renderFormConfig === 'function') {
    try {
      renderFormConfig();
    } catch (e) {}
  }
...
function renderAll() {
  ...
  renderBizDash();
  renderInvoices();
  if (typeof renderContractTemplates === 'function') renderContractTemplates();
...
function renderClients() {
  ...
  if (!list.length) {
    tbody.innerHTML =
      '<tr><td colspan="4" style="color:var(--muted);text-align:center;padding:24px">Még nincs ügyfél. Amint beérkezik egy megrendelés, az ügyfél automatikusan megjelenik itt.</td></tr>';
    return;
  }
...
function addRevenue() {
  ...
  if (!date) {
    uiAlert('Add meg a dátumot!');
    return;
  }
  if (!source) {
    uiAlert('Add meg az ügyfelet / forrást!');
    return;
  }
  if (!amount) {
    uiAlert('Add meg az összeget!');
    return;
  }
...
function renderRevenues() {
  ...
  const rvg = document.getElementById('rv-sum-gross');
  if (rvg) rvg.textContent = fmt(gross);
  const rvgs = document.getElementById('rv-sum-gross-sub');
  if (rvgs) rvgs.textContent = yearRows.length + ' ' + L('tétel', 'items');
  ...
  tbody.innerHTML =
    rows.length === 0
      ...
      : rows
          ...
          <td>${r.invoiceId || r.orderId ? '<span style="color:var(--muted);font-size:11px">Automatikusan könyvelt</span>' : `<button class="btn btn-danger btn-sm" onclick="deleteRevenue('${r.id}')">Törlés</button>`}</td>
        </tr>`;
          })
          .join('');
}
...
function renderExpenses() {
  ...
  document.getElementById('ex-sum-month-sub').textContent =
    inMonth.length + ' ' + L('tétel', 'items') + ' — ' + fmtMonth(month);
  document.getElementById('ex-avg-month').textContent = fmt(
    activeMonths ? sumYear / activeMonths : 0
  );
  ...
  document.getElementById('ex-sum-year-sub').textContent =
    yearRows.length + ' ' + L('tétel', 'items');
  ...
  if (o.status === 'kifizetve') {
    if (idx === -1) {
      state.bizIncome.push({
        ...
        date: o.paidDate || now(),
        source: o.name,
        cat: 'Weboldal projekt',
        amount: orderPriceHuf(o),
...
function reconcileOrderIncomes() {
  ...
  state.bizIncome = state.bizIncome.filter(
    (r) => !r.orderId || state.orders.some((o) => o.id === r.orderId && o.status === 'kifizetve')
  );
}
function selectOrderClientType(type) {
  ...
  if (!pb || !bb) return;
  if (type === 'Vállalkozó') {
    bb.style.background = 'var(--primary)';
    bb.style.color = '#fff';
    bb.className = 'btn';
    pb.style.background = '';
    pb.style.color = '';
    ...
    bb.style.background = '';
    bb.style.color = '';
    bb.className = 'btn btn-secondary';
  }
}
function addOrder() {
...
async function deleteOrder(id) {
  if (
    !(await uiConfirm(
      'Biztosan törlöd ezt a megrendelést? A hozzá könyvelt bevétel-tétel is törlődik.',
      {
        title: 'Megerősítés'
      }
    ))
...
function renderOrders() {
  ...
  const _oac = document.getElementById('or-active-count');
  if (_oac) _oac.textContent = leadsCount + ' ' + L('db', 'pcs');
  ...
  if (withDeadline.length) {
    ...
    nd.textContent = '—';
    nd.style.color = '';
    nds.textContent = 'Nincs aktív határidő';
  }
  const STATUS_CLASS = {
    folyamatban: 's-yellow',
    teszt: 's-cyan',
    eles: 's-green',
    torolve: 's-red',
    atadva: 's-purple',
  ...
  const statusSelect = (o) => {
    ...
    return (
      `<select class="status-select ${cls}" onchange="setOrderStatus('${o.id}', this.value);this.className='status-select ${STATUS_CLASS[this.value] || 's-gray'}'">` +
      ORDER_STATUS_KEYS.map((k) => {
        const v = ORDER_STATUS[k] || {
          label: k
        };
...
function renderBizDash() {
  ...
  const deadlineUpcoming = state.orders.filter(
    (o) => o.deadline && o.status !== 'torolve' && o.status !== 'eles' && o.deadline >= today
  ).length;
  const _set = (id, v) => {
    const el = document.getElementById(id);
  ...
  const monthData = months.map((m) => {
    let inc = 0,
      exp = 0;
    state.bizIncome.forEach((r) => {
      if ((r.date || '').slice(0, 7) === m) inc += r.amount || 0;
    });
    state.bizExpense.forEach((e) => {
      if ((e.date || '').slice(0, 7) === m) exp += e.amount;
    });
    return {
      m,
      inc,
  ...
  if (barCanvas) {
    ...
    monthData.forEach(({ m, inc, exp }, i) => {
      ...
      let label = locDate(new Date(m + '-01T00:00:00'), {
        month: 'long'
      });
  ...
  if (donutCanvas) {
    const STATUS_COLORS = {
      folyamatban: CH.amber,
      teszt: CH.teal,
      eles: CH.inc,
      torolve: CH.exp
    };
    const STATUS_LABELS = {
      folyamatban: 'Fejlesztés alatt',
      teszt: 'Teszt verzió',
      eles: 'Élesben',
      torolve: 'Törölve'
    };
    ...
    const segments = Object.entries(counts)
      .map(([k, v]) => ({
        ...
        value: v,
        color: STATUS_COLORS[k] || CH.muted
      }))
      .filter((s) => s.value > 0);
    ...
    if (legend) {
      legend.innerHTML =
        ...
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;font-size:12px">
          ...
          .join('') || '<div style="color:var(--muted);font-size:12px">Még nincs projekt.</div>';
    }
  }
  const cfBody = document.getElementById('bd-cashflow');
  let anyData = false;
  cfBody.innerHTML = monthData
    .map(({ m, inc, exp }) => {
  ...
  if (!anyData)
    cfBody.innerHTML =
      '<tr><td colspan="4" style="color:var(--muted)">Még nincs rögzített forgalom.</td></tr>';
    ...
    .forEach((inv) => {
      ...
      if (!exists) {
        const total = (inv.items || []).reduce(
          (s, it) => s + (it.qty || 1) * (it.unitPrice || 0),
          0
        );
        state.bizIncome.push({
          id: uid(),
...
function invTotalHuf(inv) {
  ...
  return inv && inv.currency === 'EUR'
    ? Math.round(net * (Number(inv.fxRate) || eurHufRate()))
    : net;
}
function invNextNum() {
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  body{font-family:'Inter',-apple-system,'Segoe UI',sans-serif;font-size:13px;color:#171c28;background:#eef1f8;padding:48px 40px}
  ...
  .dates{display:flex;gap:32px;flex-wrap:wrap;padding:14px 0;border-top:1px solid #dbe2f0;border-bottom:1px solid #dbe2f0;margin-bottom:26px}
  .dlabel{font-size:9.5px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:#5d6b85;margin-bottom:3px}
  .dval{font-size:13.5px;font-weight:600}
  table{width:100%;border-collapse:collapse;margin-bottom:6px}
  thead th{border-bottom:2px solid #171c28;padding:9px 10px;font-size:9.5px;font-weight:700;text-transform:uppercase;letter-spacing:.8px;color:#5d6b85;text-align:left}
  tbody td{padding:12px 10px;border-bottom:1px solid #e4eaf5;font-size:13px}
...
<div class="sheet">
  ...
  ${inv.sellerBank ? `<div class="pay">Fizetés átutalással · Bankszámlaszám: <strong style="color:#171c28">${esc(inv.sellerBank)}</strong></div>` : ''}
...
(function () {
  ...
  function observeAll() {
    ...
    document.querySelectorAll('.scroll-table:not(.cashflow-table) tbody').forEach((tb) =>
      mo.observe(tb, {
        childList: true
      })
    );
